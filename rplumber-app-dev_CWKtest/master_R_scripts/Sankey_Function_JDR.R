...
library(matsbyname) ## Matt's package for matrix operation
library(matsindf)   ## Matt's package for matrix operation
library(Recca)      ## Matt's package for matrix operation
...
sankey_json <- function(region_id, p_solar, p_nuclear, p_hydro, p_wind, p_geo, p_ng, p_coal, p_bio, p_petrol, r_sh_e, r_wh_e, r_ck_e, c_sh_e, c_wh_e, c_ck_e, ldv_elec, ldv_petrol, ldv_ethanol, trans_other_other, trans_other_petrol, trans_other_ng){
  ...
  if (!(as.numeric(region_id) %in% region)){
    ...
    U <- read.csv(paste0("./EIoFRegion_2050Sankey_Input_Files_20191024_NumberLabels/U", region_id, ".csv"), row.names = 1) %>%
      as.matrix() %>% setrowtype("Products") %>% setcoltype("Industries")
    U <- U/1e15 # convert to QUADS
    V <- read.csv(paste0("./EIoFRegion_2050Sankey_Input_Files_20191024_NumberLabels/V", region_id, ".csv"), row.names = 1) %>%
      as.matrix() %>% setrowtype("Industries") %>% setcoltype("Products")
    V <- V/1e15 # convert to QUADS
    Y <- read.csv(paste0("./EIoFRegion_2050Sankey_Input_Files_20191024_NumberLabels/Y", region_id, ".csv"), row.names = 1) %>% 
      as.matrix() %>% setrowtype("Products") %>% setcoltype("Industries")
